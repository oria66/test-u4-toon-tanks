...
void UHealthComponent::BeginPlay()
{
	...
	ATankGameMode = Cast<ATankGameModeBase>(UGameplayStatics::GetGameMode(GetWorld()));
...
void UHealthComponent::TakeDamage(AActor *DamagedActor, float Damage, const UDamageType *DamageType, AController *InstigatedBy, AActor * DamageCause) 
{
	...
	if(Health <= 0)
	{
		if(ATankGameMode)
		{
			ATankGameMode->ActorDied(GetOwner());
		}
		else
		{
			UE_LOG(LogTemp, Warning, TEXT("No Game Mode detected"));
		}
	}
}
